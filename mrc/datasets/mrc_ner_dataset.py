...
class MRCNERDataset(Dataset):
    ...
    def __init__(self, json_path, tokenizer: BertWordPieceTokenizer, max_length: int = 128, possible_only=False,
                 is_chinese=False, pad_to_maxlen=False,file=None):
    ...
    def __getitem__(self, item):
        ...
        #print(sample_idx,label_idx,file=self.file)
        query = data["query"]
        ...
        #print(len(tokens),file=self.file)
        #print(offsets,file=self.file)
        ...
        for token_idx in range(len(tokens)):
            ...
            lookup_start.append(token_start)
            lookup_end.append(token_end)
        
        new_start_positions = []
        for start in start_positions:
            if start in origin_offset2token_idx_start.keys():
                new_start_positions.append(origin_offset2token_idx_start[start])
            else:
                continue
                #for i in range(len(lookup_start)):
                #    if start > lookup_start[i]:
                #        break
                #new_start_positions.append(origin_offset2token_idx_end[lookup_end[i-1]]+1)
        
        new_end_positions = []
        for end in end_positions:
            if end in origin_offset2token_idx_end.keys():
                new_end_positions.append(origin_offset2token_idx_end[end])
            else:
                continue
                #for i in range(len(lookup_end)):
                #    if end > lookup_end[i]:
                #        break
                #new_end_positions.append(origin_offset2token_idx_start[lookup_start[i]]+1)

        label_mask = [
            (0 if type_ids[token_idx] == 0 or offsets[token_idx] == (0, 0) else 1)
            for token_idx in range(len(tokens))
        ]
        #print('label_mask',label_mask,file=self.file)
...
def run_dataset(file=None):
    ...
    #json_path =  "zh_msra/mrc-ner.train.small"
    json_path = "../ner2mrc/zh_mrc/reformat80/mrc_format/mrc-ner.train"# "zh_msra/mrc-ner.train.small"
    ...
    dataset = MRCNERDataset(json_path=json_path, tokenizer=tokenizer,
                            is_chinese=is_chinese,file=file)
    ...
    for batch in dataloader:
        for tokens, token_type_ids, start_labels, end_labels, start_label_mask, end_label_mask, match_labels, sample_idx, label_idx in zip(*batch):
            ...
            if not start_positions:
                continue
            print("="*20,file=file)
            print(f"len: {len(tokens)}", tokenizer.decode(tokens, skip_special_tokens=False),file=file)
            #print(start_positions,end_positions)
            for start, end in zip(start_positions, end_positions):
                print(str(sample_idx.item()), str(label_idx.item()) + "\t" + tokenizer.decode(tokens[start: end+1]),file=file)
