...
def train_and_predict(only_label, train_data):
    model = NERModel('bert', f'bert-base-chinese', 
        labels=label_list, args={'train_batch_size':16, 'overwrite_output_dir': True, 'output_dir':'output/ner/bert_sim/{only_label}', 'reprocess_input_data': True, 'num_train_epochs': 15})
    # Train the model
    ...
    #test_df.to_csv('output_file/bert_Simplified_{only_label}.csv', index = None, encoding='utf-8-sig')
    return test_df

def trans_label(row, label_name):
  if row != label_name:
      return 'O'
  else:
      return row

whole_label = ['name', 'location', 'time', 'contact', 'id', 'profession', 'biomarker', 'family', 
...
for i in whole_label:
    ...
    train_data_each['labels'] = train_data_each['labels'].apply(trans_label, args=(i), 1)
